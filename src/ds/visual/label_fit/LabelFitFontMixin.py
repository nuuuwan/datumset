class LabelFitFontMixin:
    ...
    def _rect_px(rect_w, rect_h, ax, renderer):
        ...
        px_w = axes_bb.width * rect_w / max(xlim[1] - xlim[0], 1e-9)
        px_h = axes_bb.height * rect_h / max(ylim[1] - ylim[0], 1e-9)
        return px_w, px_h

    @staticmethod
    def _char_line_px(fontsize, ax, renderer):
        sample = ax.text(0, 0, "n" * 10, fontsize=fontsize)
    ...
    def char_budget(cls, rect_w, rect_h, fontsize, ax, renderer):
        ...
        budget = px_w * cls.LABEL_MARGIN / max(char_px, 1e-9)
        if line_px > px_h:
            budget *= px_h / line_px
        return int(budget)
